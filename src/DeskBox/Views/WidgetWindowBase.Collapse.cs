...
public abstract partial class WidgetWindowBase
{
    ...
    private void CollapseAnimationRendering()
    {
        ...
        double wallElapsedMs = Stopwatch.GetElapsedTime(_collapseAnimationStarted, frameTimestamp).TotalMilliseconds;
        double wallProgress = Math.Clamp(wallElapsedMs / Math.Max(1, _collapseAnimationDurationMs), 0, 1);
        if (!_collapseAnimationPacing.ShouldSubmit(
                wallElapsedMs,
                frameBudgetMs,
                force: wallProgress >= 1))
        {
            return;
        }
        ...
        double elapsedMs = wallElapsedMs;
    ...
    /// <summary>
    /// Adapts the optional user-selected animation cap and the session fallback
    /// ladder without changing the elapsed-time progress of the morph.
    /// </summary>
