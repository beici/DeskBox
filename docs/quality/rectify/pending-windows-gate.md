...
- 桌面图标矩形取的是 LVIR_BOUNDS（图标 + 文字标签的整体外框），因此贴到长文件名图标旁边时，参考边界会包含文字宽度。如需只按图标图形对齐，可改用 LVIR_ICON，属可配置项，本批未开。**【已定 2026-09-13：用户确认维持现状（含文字），此选项关闭】**
...
- 对置边（上/下 或 左/右）同时被编辑时，二者可能互相矛盾（格子高度固定），当前按 左→上→右→下 的顺序应用，**后应用的一侧生效**。属 DEF-062 既有语义，本批未改。**【已定 2026-09-13：用户确认接受该语义（连同「最近邻居随移动换对象」），不改为报错/折中】**
- 「最近邻居」是按当前位置解析的：移动之后可能有另一个更近的对象接管该侧，因此保存后显示的数值可以与刚输入的值不同（例如输入 60 落位后显示 55）。这是参考系的真实结果而非丢失输入，焦点框在编辑期间不会被改写。

---

## 边距测量主体批次（DEF-064，2026-09-06）

> 门禁：Debug 构建 0 错误 20 警告（均为既有）；`dotnet test -p:Platform=x64` **3254/3254 全绿**。

### 已完成核验（实机复现 → 修复 → 同一路径复测）

- [x] **复现用户现象**：胶囊 `(2169,590,391,52)` 悬停展开成 `391x345`（底边 935），「下」退回工作区边缘得 `1380−935=445`，与用户截图数值一致
- [x] **修复后同一胶囊**：verbose 日志 `subject=2169,740 391x52 live=2169,740 391x311` → `左=18/Widget 上=22/Widget 右=0/WorkArea 下=25/Widget`；界面四个副标题依次为「最近的格子 / 最近的格子 / 最近的格子 / 屏幕边缘」
- [x] **应用路径同源**：输入 `下=60`，胶囊精确落到 y=705（=817−60−52）；随后输入 25 并保存，回到原位 y=740 并持久化
- [x] **展开态与收起态一致**：编辑期间指针移到编辑器窗口后胶囊自动收起（live 变回 391x52），四个数值与展开态解析结果相同，不再跳变
- [x] **新增可取证日志**：`[WidgetMargin] Reference resolve hwnd= subject= live= widgets= neighbours= tolerance= left/top/right/bottom=距离/参考类型`（`DESKBOX_VERBOSE_LOG=1`）
