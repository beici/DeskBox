...
- **审计曾止于 ILC 原生链接，根因是环境变量 `OS` 被置空（并非宿主能力边界）**：`Microsoft.NETCore.Native.Publish.targets(63,5): error : Cross-OS native compilation is not supported`。该守卫写作 `'$(OS)' != 'Windows_NT' and '$(_targetOS)' == 'win'`，而 MSBuild 的 `$(OS)` 直接继承环境变量 `OS`；沙箱把它置空（属性探测：未设时 `"OS": ""`，设 `$env:OS = "Windows_NT"` 后为 `"Windows_NT"`），故这条**分类式的「不支持」报错被误触发**；补上该变量后审计即越过此步进入 `Generating native code`。**更正**：此前本文把该报错归因为「沙箱宿主的能力边界」，属误判。同族发现：沙箱同样置空 `PATHEXT`（仅剩 `.CPL`，使 PowerShell 无法把 `.exe` 当可执行文件）与 `PROGRAMFILES*` / `PROGRAMDATA`（使 `vswhere` 枚举不到 VS 实例）。另：仓库内 `$(OS)` **零引用**，仓库外仅 `Microsoft.DotNet.ILCompiler` 与 `Microsoft.TestPlatform.TestHost` 读它；后者只影响 `testhost.exe` 这个 Content 项的拷贝（.NET 项目走 `testhost.dll`，不影响 `dotnet test` 结果），前者的 `_hostArchitecture` 在两种取值下均为 `x64`，故此前的 Release 构建与 4226/4226 套件结论**仍然有效**。
...
- **审计本体已完整跑通（`RESULT=SUCCESS`）**：在冻结工作区（`git status` 为空）下重跑，`summary.json` 记录 `sourceStableDuringAudit=true`、`workingTreeFingerprintBefore == workingTreeFingerprintAfter`、`gitCommit=99be316`、`auditProfileVersion=63`、`durationMilliseconds=349568`（约 5 分 50 秒）。**35 枚 WMC1510 钉版（31 枚 `Expected` + 4 枚 `Maximum`）取值全为 742，35 枚 `Actual` 也全为 742，一一相符**；`stage5B4B1Expected/ActualBindableViewModelPropertyCount` 均为 351；`unexpectedWarningCodes=[]`、`alwaysThrowMessages=[]`，全部 `*SourceWarningMessages` / `*MissingBindablePatterns` 为空；`warningCodes` 恰为白名单六项（`WMC1510=742`、`CS8602=14`、`CS8601/CS0414/CS0169/CS0108=2`）；`targetedWarningCounts` 的 `MVVMTK0045`/`CsWinRT1028` 均为 0。产出 `publishFileCount=45`、`publishBytes≈102 MB`。**门禁结论自此为审计本体实测，不再是回放推断。**
- **跑审计必须冻结工作区**：脚本在 `:248` 与 `:8432` 各取一次工作树指纹，`:9247` 不一致即抛 `The repository changed while the AOT audit was running`。首次完整跑通时因在运行中修改了本文档（06:13:10，快照后时间 06:13:31）而触发该守卫——注意 `summary.json` 写在守卫之前（`:9242-9245`），故那次失败的 summary 仍是完整的、含全部阶段结果的证据；另外脚本自带的 `Remove-Item $runRoot -Recurse -Force`（`:253`）在本沙箱会被批量删除防护拦下，需先将运行目录**改名**让开。**`.workbuddy-ai/` 已被 gitignore，向其中写文件对快照不可见**，故运行期间的记忆/笔记写入不影响审计。
